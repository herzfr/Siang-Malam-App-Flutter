...
class PoItemCreditControllerV2 extends GetxController {
  // var log = Logger();
  ...
  void onInit() async {
    super.onInit();
    listPoitem.value = Get.arguments ?? ListPoItem();
  ...
  onChangeValueAmount(String value) {
    ...
    int e = listPoitem.value.remaining ?? 0;
    if (amount > e) {
  ...
  initValueSubmit(String expenseType) {
    paymentPo.update((val) {
      val?.orderNo = int.parse(listPoitem.value.orderNo!);
  ...
  Future<void> fetchCredit() async {
    detailPurchasing.value = await PurchaseItemService.getDetailPoItemsPayment(
        listPoitem.value.orderNo!);
    // log.d(detailPurchasing.toJson());
  ...
  Future<void> pushNewPayment() async {
    isSubmit.value = true;
    bool result = await PurchaseItemService.approvePayment(
