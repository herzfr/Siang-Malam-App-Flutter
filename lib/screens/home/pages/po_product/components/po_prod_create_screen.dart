...
import 'package:siangmalam/screens/home/pages/po_product/components/panel/list_prod_panel_screen.dart';
import 'package:siangmalam/widgets/custom_input.dart';
import 'package:sliding_up_panel/sliding_up_panel.dart';
import 'package:sizer/sizer.dart';

class PoProductCreateScreen extends GetView<PoProductCreateController> {
  const PoProductCreateScreen({Key? key}) : super(key: key);
                    ...
                    children: [
                      Obx(
                        () => Padding(
                          padding: EdgeInsets.only(
                              left: 20,
                              top: controller.listProd.value.isEmpty
                                  ? 110.0
                                  : 10.0,
                              right: 20,
                              bottom: 10),
                          child: Obx(
                            () => TextFormField(
                                ...
                                onFieldSubmitted: (value) {
                                  controller.textFieldNoteController.value
                                      .text = value;
                                  controller.poProductCreate.value.note = value;
                                },
                                onChanged: (value) => {
                                      // controller.controllersNote.value.text =
                                      //     value;
                                      controller.poProductCreate.value.note =
                                          value
                                    }),
                          ),
                        ),
                        ...
                        child: Obx(
                          () {
                            return DropdownButtonFormField<int>(
                              ...
                              value:
                                  controller.poProductCreate.value.warehouseId,
                              decoration:
                        ...
                        child: Obx(
                          () {
                            return DropdownButtonFormField<int>(
                              ...
                              value: controller.poProductCreate.value.suplierId,
                              decoration:
                                  CustomFormWidget.customInputDecoration(
                                      '', 'Pilih Suplier/Toko'),
                              onChanged: (value) {
                                controller.poProductCreate.value.suplierId =
                                    value!;
                              },
                            );
                          },
                      ...
                      Padding(
                        padding: EdgeInsets.only(
                            ...
                            left: 20),
                        child: Obx(
                          () => controller.listProd.value.isEmpty
                              ? const Text(
...
AppBar buildAppBar(PoProductCreateController controller) {
  return AppBar(
    ...
    actions: [
      Obx(
        () => TextButton(
          ...
          onPressed: controller.listProd.isNotEmpty
              ? () {
                  Get.defaultDialog(
                      title: "Buat?",
                      middleText:
...
class ListCreate extends GetView<PoProductCreateController> {
  ...
  Widget build(BuildContext context) {
    return Obx(
      () => ListView.builder(
        ...
        itemCount: controller.listProd.value.length,
        itemBuilder: (context, index) {
          controller.controllersCost.value.add(TextEditingController(
              text: controller.listProd.value[index].cost.toString()));
          controller.controllersQty.value.add(TextEditingController(
              text: controller.listProd.value[index].quantity.toString()));
                      ...
                      child: Stack(
                        children: [
                          ...
                          Align(
                            alignment: Alignment.center,
                            child: Text(
                              controller.listProd.value[index].name ?? '',
                  ...
                  Padding(
                      ...
                      child: Row(
                        children: [
                          ...
                          Text(
                            controller.listProd.value[index].unit ?? '',
