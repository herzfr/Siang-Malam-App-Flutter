...
class SpaFormScreen extends GetView<SpaFormController> {
  static String routeName = " /stock-product-adjustment";
  const SpaFormScreen({Key? key}) : super(key: key);
        ...
        child: Column(
          children: [
            Padding(
              ...
              child: Obx(() {
                if (controller.warehouseIn.value.branchId != null) {
                  return DropdownButtonFormField<ListWarehouse>(
                    // key: controller.dropStatus,
                    items: controller.warehouseDD,
                    value: controller.warehouseIn.value,
                    decoration: CustomFormWidget.customInputDecoration(
                        '', 'Dapur/Gudang'),
                    onChanged: controller.warehouseIn.value.branchId != null
                        ? (value) {
                            controller.warehouseIn.value =
                                value ?? ListWarehouse();
                          }
                        : null,
                  );
            ...
            Obx(
              () => controller.prodAdjstList.isNotEmpty
                  ? const Text(
  ...
  ListView initBodyCard(BuildContext context) {
    return ListView.builder(
      itemCount: controller.prodAdjstList.length,
      itemBuilder: (context, index) => Padding(
        padding: EdgeInsets.symmetric(vertical: 0.5.h),
        child: Dismissible(
          key: Key(controller.prodAdjstList[index].stockId.toString()),
          direction: DismissDirection.endToStart,
          onDismissed: (direction) {
            controller.prodAdjstList.removeAt(index);
            if (controller.prodAdjstList.isEmpty) {
          ...
          children: [
            Expanded(
              ...
              child: Column(
                ...
                children: [
                  Text(
                    controller.prodAdjstList[idx].prodName ?? '',
                    style: TextStyle(
                    ...
                    children: [
                      Expanded(
                        ...
                        child: Obx(
                          () => Text(
                            'x' +
                                controller.prodAdjstList[idx].quantity
                                    .toString() +
                                " " +
                                controller.prodAdjstList[idx].unit!,
                            style: TextStyle(
          ...
          Expanded(
            ...
            child: Obx(
              () => ElevatedButton(
                ...
                onPressed: controller.prodAdjstList.isNotEmpty &&
                        controller.warehouseIn.value.branchId != null
                    ? () {
                        Get.defaultDialog(
                            title: "Penyesuaian?",
                            middleText:
                                "Penyesuaian produk ke ${controller.warehouseIn.value.name}, lanjutkan?",
